...
key-points:
  - Software development is becoming a central aspect in research projects in the (digital) humanities, but those projects often struggle with the implementation of modern engineering practices, which could be helpful to ensure the sustainability of the developed software and speed up the development process itself.
  - The development of a XML schema for a scholarly edition project is a complex task, which requires a lot of effort and time, but if done right can be a solid foundation for the entire project.
date: 03-17-2024
...
Nowadays, software is a central component of every research project. Since the establishment of personal computers digital tools are used for a wide range of tasks, from simple text processing (creation) to machine assisted recognition of patterns in damaged papyri. However, projects, in particular those that produce a (digital) scholarly edition, are rarely not only users of existing tools, but also creators of them. Starting from the development / customization of own data formats to the implementation of often complex web applications for presentation. It is not uncommon for the tools developed in this context to be "quick hacks" rather than well-designed software projects. In many cases, this is not a problem at all. The duration of humanities research projects is often manageable (e.g. 3 years). The software developed must first and foremost contribute to achieving the project goals. Adaptation or subsequent use are not usually intended. However, this always becomes a problem if this is precisely the intention or if the corresponding research projects are scheduled for a longer term. If this is the case, then quick solutions often become a problem and are not really FAIR for either internal or external subsequent use. Not least for this reason, this realisation is increasingly becoming the subject of discussion in the digital humanities community under the heading of "research software engineering".^[Manuel Burghardt and Claudia Müller-Birn organised a workshop specifically on this topic at the 50th Annual Conference of the German Informatics Society, see @informatik_software_2019.] The following paper describes the practical experiences from the perspective of a long-term editorial project and explores possibilities for sustainable development practice through the use of modern practices that have long been firmly established outside the academic world.
